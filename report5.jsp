<%
CREATE OR REPLACE FUNCTION fourth()
RETURNS trigger AS
$$
BEGIN
    IF NEW.grade != 'A' OR NEW.grade != 'B' OR NEW.grade != 'C' OR NEW.grade != 'D' OR NEW.grade != 'F' 
    THEN RAISE EXCEPTION 'Grade % is inapplicable', NEW.grade;
    ELSIF IS NOT NULL(SELECT s_ssn FROM past_classes WHERE s_ssn = NEW.s_ssn AND co_number = NEW.co_number AND section_id = NEW.section_id) 
    THEN RAISE EXCEPTION 'Student % already took couse % section %', NEW.s_ssn, NEW.co_number, NEW.section_id;
    ELSE 
      INSERT INTO past_classes(s_ssn, co_number, quarter, section_id, grade) VALUES(NEW.s_ssn, NEW.co_number, NEW.quarter, NEW.section_id, NEW.grade);
    END IF;
END;
$$
LANGUAGE plpgsql;

CREATE TRIGGER handle_grade
BEFORE INSERT ON past_classes
FOR EACH ROW EXECUTE PROCEDURE fourth(); 

CREATE OR REPLACE FUNCTION fifth()
RETURNS trigger AS
$$
BEGIN
    IF NEW.grade != 'A' OR NEW.grade != 'B' OR NEW.grade != 'C' OR NEW.grade != 'D' OR NEW.grade != 'F' 
...
FOR EACH ROW EXECUTE PROCEDURE fifth(); 
...
<%
        ...
        if (action != null && action.equals("get")) {
            ...
            String query5 = "(SELECT COUNT(pc.grade) AS A FROM faculty f, student s, past_classes pc, class c WHERE pc.s_ssn = s.s_ssn AND c.f_name = f.f_name AND pc.co_number = c.co_number AND c.co_number = '" + co_number + "' AND (pc.grade = 'A' OR pc.grade = 'A+' OR pc.grade = 'A-')) UNION ALL (SELECT COUNT(pc.grade) AS B FROM faculty f, student s, past_classes pc, class c WHERE pc.s_ssn = s.s_ssn AND c.f_name = f.f_name AND pc.co_number = c.co_number AND c.co_number = '" + co_number + "' AND f.f_name = '" + f_name + "' AND (pc.grade = 'B' OR pc.grade = 'B+' OR pc.grade = 'B-')) UNION ALL (SELECT COUNT(pc.grade) AS C FROM faculty f, student s, past_classes pc, class c WHERE pc.s_ssn = s.s_ssn AND c.f_name = f.f_name AND pc.co_number = c.co_number AND c.co_number = '" + co_number + "' AND  (pc.grade = 'C' OR pc.grade = 'C+' OR pc.grade = 'C-')) UNION ALL (SELECT COUNT(pc.grade) AS D FROM faculty f, student s, past_classes pc, class c WHERE pc.s_ssn = s.s_ssn AND c.f_name = f.f_name AND pc.co_number = c.co_number AND c.co_number = '" + co_number + "' AND f.f_name = '" + f_name + "' AND (pc.grade = 'D' OR pc.grade = 'D+' OR pc.grade = 'D-')) UNION ALL (SELECT COUNT(pc.grade) AS Other FROM faculty f, student s, past_classes pc, class c WHERE pc.s_ssn = s.s_ssn AND c.f_name = f.f_name AND pc.co_number = c.co_number AND c.co_number = '" + co_number + "' AND f.f_name = '" + f_name + "' AND (pc.grade = 'F' OR pc.grade = 'IN'))";
            out.println(query5);
            rs_eleven = statement11.executeQuery(query5);

            String query4 = "(SELECT COUNT(pc.grade) AS A FROM faculty f, student s, past_classes pc, class c WHERE pc.s_ssn = s.s_ssn AND c.f_name = f.f_name AND pc.co_number = c.co_number AND pc.section_id = c.section_id AND c.co_number = '" + co_number + "' AND f.f_name = '" + f_name + "' AND (pc.grade = 'A' OR pc.grade = 'A+' OR pc.grade = 'A-')) UNION ALL (SELECT COUNT(pc.grade) AS B FROM faculty f, student s, past_classes pc, class c WHERE pc.s_ssn = s.s_ssn AND c.f_name = f.f_name AND pc.co_number = c.co_number AND c.co_number = '" + co_number + "' AND f.f_name = '" + f_name + "' AND pc.section_id = c.section_id AND (pc.grade = 'B' OR pc.grade = 'B+' OR pc.grade = 'B-')) UNION ALL (SELECT COUNT(pc.grade) AS C FROM faculty f, student s, past_classes pc, class c WHERE pc.s_ssn = s.s_ssn AND c.f_name = f.f_name AND pc.co_number = c.co_number AND c.co_number = '" + co_number + "' AND f.f_name = '" + f_name + "' AND c.year = '" + year + "' AND c.section_id = pc.section_id AND (pc.grade = 'C' OR pc.grade = 'C+' OR pc.grade = 'C-')) UNION ALL (SELECT COUNT(pc.grade) AS D FROM faculty f, student s, past_classes pc, class c WHERE pc.s_ssn = s.s_ssn AND c.f_name = f.f_name AND pc.co_number = c.co_number AND c.co_number = '" + co_number + "' AND f.f_name = '" + f_name + "' AND pc.section_id = c.section_id AND (pc.grade = 'D' OR pc.grade = 'D+' OR pc.grade = 'D-')) UNION ALL (SELECT COUNT(pc.grade) AS Other FROM faculty f, student s, past_classes pc, class c WHERE pc.s_ssn = s.s_ssn AND c.f_name = f.f_name AND pc.co_number = c.co_number AND c.co_number = '" + co_number + "' AND f.f_name = '" + f_name + "' AND pc.section_id = c.section_id AND (pc.grade = 'F' OR pc.grade = 'IN'))";
